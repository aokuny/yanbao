...
-(void)setFrameWith:(BOOL)isHaveNavControler{
    CGFloat toolViewX = 0;
    CGFloat toolViewH = _pickeviewHeight+CLToobarHeight;
    CGFloat toolViewY ;
...
-(void)setUpPickView{
    ...
    pickView.frame=CGRectMake(0, CLToobarHeight, pickView.frame.size.width, pickView.frame.size.height);
...
-(void)setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode{
    ...
    datePicker.frame=CGRectMake(0, CLToobarHeight, datePicker.frame.size.width, datePicker.frame.size.height);
...
-(void)setToolbarWithPickViewFrame{
    _toolbar.frame=CGRectMake(0, 0,[UIScreen mainScreen].bounds.size.width, CLToobarHeight);
}

-(void) setDefaultSelectedWithRowIndex:(NSInteger) defaultRowIndex{
    [_pickerView selectRow:defaultRowIndex inComponent:0 animated:YES];
    _resultString=_plistArray[defaultRowIndex];
}
